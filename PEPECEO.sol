...
// 🌎 Website: https://www.pepeceo.space
...
contract PEPECEPO is ERC20, Ownable {
    using Address for address;
    using Address for address payable;
    
    bool public antiBotEnabled;

    uint256 public  rewardFeeOnBuy      = 4;
    uint256 public  rewardFeeOnSell     = 4;
    ...
    uint256 private _totalFeesOnBuy     = 10;
    uint256 private _totalFeesOnSell    = 10;

    address public marketingWallet = 0x6dB3455095f2140eD9B8F6b38294635C281524F6;
    address public devWallet = 0xD969DD00B96ad5C782c7e37602271dBE7FA81C3B;
    ...
    event UpdateBuyFees(uint256 marketingFeeOnBuy, uint256 devFeeOnBuy, uint256 rewardFeeOnBuy);
    event UpdateSellFees(uint256 marketingFeeOnSell, uint256 devFeeOnSell, uint256 rewardFeeOnSell);
    ...
    constructor() ERC20("PEPECEO Inu", "PEPECEO") 
    {
    	...
    	dividendTracker = new DividendTracker(1_000_000_000_000 / 50000, rewardToken);
        ...
        dividendTracker.excludeFromDividends(address(pinkLock));

        _isExcludedFromFees[owner()] = true;
        _isExcludedFromFees[DEAD] = true;
        _isExcludedFromFees[address(devWallet)] = true;
        _isExcludedFromFees[address(this)] = true;
        _isExcludedFromFees[address(pinkLock)] = true;
    }
    ...
    function updateBuyFees(uint256 _marketingFeeOnBuy,uint256 _devFeeOnBuy ,uint256 _rewardFeeOnBuy) external onlyOwner {
        require(
            _marketingFeeOnBuy + _devFeeOnBuy + _rewardFeeOnBuy <= 25,
            "Fees must be less than 25%"
        );
        rewardFeeOnBuy    = _rewardFeeOnBuy;
        marketingFeeOnBuy = _marketingFeeOnBuy;
        devFeeOnBuy = _devFeeOnBuy;
        _totalFeesOnBuy   = marketingFeeOnBuy + devFeeOnBuy + rewardFeeOnBuy;
        emit UpdateBuyFees(_marketingFeeOnBuy, _devFeeOnBuy, _rewardFeeOnBuy);
    }

    function updateSellFees(uint256 _marketingFeeOnSell, uint256 _devFeeOnSell, uint256 _rewardFeeOnSell) external onlyOwner {    
        require(
            _marketingFeeOnSell + _devFeeOnSell + _rewardFeeOnSell <= 25,
            "Fees must be less than 25%"
        );
        rewardFeeOnSell    = _rewardFeeOnSell;
        devFeeOnSell = _devFeeOnSell;
        marketingFeeOnSell = _marketingFeeOnSell;
        _totalFeesOnSell   = marketingFeeOnSell + devFeeOnSell + rewardFeeOnSell;
        emit UpdateSellFees(_marketingFeeOnSell, _devFeeOnSell, _rewardFeeOnSell);
    }
    ...
    function changeDevWallet(address _devWallet) external onlyOwner {
        require(_devWallet != devWallet, "Devwallet is already that address");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            if(contractTokenBalance > 0 && bnbShare > 0) {
                ...
                if(devShare > 0) {
                    ...
                    payable(devWallet).sendValue(devBNB);
                }
                if(rewardShare > 0) {
        ...
        if(((_isExcludedFromFees[from] || _isExcludedFromFees[to]) ||
            ...
            _totalFees = block.number <= antiBotBlockEnd? 25 :_totalFeesOnBuy;
        } else {
            _totalFees = block.number <= antiBotBlockEnd? 25 : _totalFeesOnSell;
        }
